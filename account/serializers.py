...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if old == password:
            raise serializers.ValidationError('Старый и новый пароль не должны совпадать')
        if password != password_confirm:
            raise serializers.ValidationError('Пароли не совпадают')
    ...
    def set_new_password(self):
        new_pass = self.validated_data.get('new password')
        user = self.context.get('request').user
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def gen_new_password(self):
        ...
        send_mail('Восстановления пароля', f'Ваш новый пароль:{new_pass}', 'test@gmail.com', [user.email])
